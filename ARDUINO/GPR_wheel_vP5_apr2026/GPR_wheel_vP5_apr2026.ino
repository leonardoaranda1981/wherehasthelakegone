//////////////////////////////////
//     based on:                //
//     <c> Mirel Paun 2020      //
//////////////////////////////////

//sampling period is 224.38 us
//stored file format - the first number (2 bytes) contains the speed index, the following contain the time domain samples
#define MCP4725_Address 96            //DAC address
#define pin_ADC A15                   //ADC input pin
#define pin_BAT A13                   //Battery voltage input pin
#define WHITE     0xFFFF              //color
#define BLACK     0x0000              //color
#define RED       0xF800              //color
#define GREEN     0x07E0              //color
#define DKBLUE    0x000D              //color
#define button_OK 6
#define button_down 5
#define button_up 4
#define HallSensor 3
#define SDC_CS 53                     //SD card control pin
#define NUMPIXELS 16
#define pinNeopixel 12
#define pinNeopixelV 13
bool firstRun = false;

#include <Wire.h>
#include "arduinoFFT.h"
#include "Free_Fonts.h"
#include <SD.h>
#include <Adafruit_GFX.h>      // Include Adafruit Core graphics library
#include <Adafruit_ST7789.h>   // Include Adafruit Hardware-specific library for ST7789
#include <SPI.h>               // Include Arduino SPI library
#include <TinyGPS.h>//incluimos TinyGPS
#include <Adafruit_NeoPixel.h>
#ifdef __AVR__
#include <avr/power.h> // Requerido para Adafruit Trinket de 16 MHz
#endif
//TFT_HX8357 tft = TFT_HX8357();
// Define ST7789 display pin connection
#define TFT_CS     10
#define TFT_RST     7   // Or set to -1 and connect to Arduino RESET pin
#define TFT_DC      8

Adafruit_ST7789 tft = Adafruit_ST7789(TFT_CS, TFT_DC, TFT_RST);
arduinoFFT FFT = arduinoFFT();

File file;
File trackLogFile;
int logIndex  = 0; 
String logName = "";

TinyGPS gps;//Declaramos el objeto gps


const word battery_level[3] = {384, 392, 401};   //battery voltage threshold levels: low - med1 - med2 (V_bat/5V*1023*0.4)

//GPR global parameters
const double GPR_horizontal_step = 0.12;  //GPR horizontal step [m]
const double bandwidth = 587000000;       //GPR bandwidth [Hz]; 910 MHz - 323 MHz
const double max_amplitude = 12000;  	    //maximum FFT magnitude value (for scaling)
const word no_of_samples = 256;           //no. of time domain samples
const word cable_offset = 9;              //offset cables + antennas in no. of resolution cells

//menus
const char *propagation_media[] = {"Air - 0.3 m/ns", "Ice - 0.16 m/ns", "Dry sand - 0.15 m/ns", "Dry soil (Granite) - 0.13 m/ns",
                                   "Limestone - 0.12 m/ns", "Asphalt - 0.11 m/ns", "Concrete - 0.1 m/ns", "Moist soil - 0.09 m/ns", "Wet soil (Silt) - 0.07 m/ns",
                                   "Saturated sand (Clay) - 0.06 m/ns", "Sweet water - 0.03 m/ns", "Salt water - 0.01 m/ns"
                                  };
const double velocities[] = {300000000, 160000000, 150000000, 130000000, 120000000, 110000000, 100000000, 90000000, 70000000, 60000000, 30000000, 10000000};
const word depths[] = {16, 32, 64, 128}; //resolution multiples
const double depth_steps[] = {10, 5, 2.5, 1, 0.5, 0.25, 0.1, 0.05, 0.025, 0.01};
const word depth_steps_decimal_places[] =  {0,  0, 1  , 0,   1,    2,   1,    2,     3,    2};
const word distances[] = {6, 12, 24, 48};

//graph constants
const word orig_x = 35, orig_y = 226, graph_width = 265, graph_height = 192; //graph origin coord. and dimensions [pixels]
//graph global variables
double max_dist, dist_step, max_depth, min_depth, depth_step, resolution; // [m]
double c; //[m/s]
word no_res_cells_horiz, no_res_cells_vert, res_cell_height, res_cell_width, no_depth_steps_decimal_places, xpos, ypos, scan_index = 0;

//antenna coupling correction (anechoic chamber acquisition)
//const word correction[no_of_samples] = {497, 497, 477, 383, 251, 163, 125, 113, 146, 210, 305, 430, 550, 682, 801, 893, 947, 964, 922, 787, 654, 569, 521, 486, 455, 446, 451, 454, 439, 409, 377, 352, 337, 332, 323, 334, 342, 354, 371, 384, 397, 410, 420, 433, 449, 468, 496, 528, 560, 596, 637, 674, 705, 726, 733, 735, 735, 738, 749, 757, 760, 754, 731, 699, 657, 597, 520, 432, 342, 264, 213, 180, 164, 164, 173, 194, 222, 252, 288, 316, 350, 390, 425, 459, 491, 522, 548, 571, 590, 606, 624, 642, 660, 681, 694, 703, 706, 701, 692, 676, 651, 623, 590, 557, 528, 501, 477, 457, 443, 433, 429, 429, 431, 433, 439, 449, 462, 476, 492, 508, 525, 543, 566, 587, 604, 609, 603, 589, 570, 547, 519, 482, 434, 376, 326, 277, 233, 194, 159, 147, 167, 224, 306, 383, 449, 503, 545, 576, 601, 611, 615, 616, 617, 617, 616, 614, 613, 609, 602, 593, 584, 577, 571, 566, 559, 553, 545, 539, 533, 528, 524, 521, 518, 515, 510, 505, 500, 496, 493, 490, 485, 480, 477, 475, 474, 475, 476, 479, 484, 490, 496, 502, 508, 514, 522, 532, 538, 542, 541, 540, 538, 536, 536, 534, 531, 525, 520, 511, 503, 497, 491, 487, 483, 479, 473, 467, 468, 468, 466, 466, 466, 466, 467, 467, 470, 468, 467, 467, 466, 466, 465, 465, 467, 468, 467, 468, 467, 471, 473, 475, 477, 480, 482, 484, 486, 489, 491, 494, 495, 497, 497, 498, 498, 499, 498, 498};

//global variables
word i, scaled_amplitude, color, samples[no_of_samples];
double real[no_of_samples], imag[no_of_samples], depth_corrected_amplitude;
boolean cont, card, GPSfixed;
 
float latitude = 19.4326077;  
float longitude = -99.133208;

Adafruit_NeoPixel pixels(NUMPIXELS, 12, NEO_GRB + NEO_KHZ800);

//------------------------------------------------------------------------
void setup(){
////////////Led indicator inicializaing
  pinMode(13, OUTPUT);
  digitalWrite(13, HIGH);
  pixels.begin();
  for (int i = 0; i < NUMPIXELS; i++) { // Para cada pixel...
    pixels.setPixelColor(i, pixels.Color(0,0, 255));
  }
  pixels.show();
  //////////////////
  Serial.begin(115200);

  if (SD.begin(SDC_CS)) {
    card = true;
    //Serial.println("SD intitialized properly and a card is present.");
  } else {
    //Serial.println("initialization failed. Things to check:");
    //Serial.println("* is a card inserted?");
  }
  GPSfixed = false;
  Serial3.begin(9600);//Iniciamos el puerto serie del gps
  while (!Serial3) {
    ; // wait for serial port to connect. Needed for native USB port only
  }

  Wire.begin();
  Wire.setClock(400000);
  tft.init(240, 320, SPI_MODE3);   // Init ST7789 240x240
  tft.fillScreen(ST77XX_BLUE);
  tft.setRotation(3);
  tft.setTextColor(ST77XX_WHITE);
  xpos = 10; // Middle of screen
  ypos = (tft.height() / 2) - 12;//tft.fontHeight(GFXFF); // Middle of screen

  pinMode(button_up, INPUT);
  pinMode(button_OK, INPUT);
  pinMode(button_down, INPUT);
  pinMode(HallSensor, INPUT);

  //
  //attachInterrupt(digitalPinToInterrupt(HallSensor), readHallSensor, CHANGE);

  tft.setCursor(xpos, ypos);
  tft.setTextSize(2);
  tft.println("Where has the lake gone?");
  tft.setCursor(xpos, ypos + 20);
  tft.setTextSize(1.5);
  tft.println("Circuit based on Mirel Paun 2020 <c>");
  delay(5000);
  tft.fillScreen(BLACK);

  //Bring DAC at 0
  Wire.beginTransmission(MCP4725_Address);
  Wire.write( 0 );        //MSB
  Wire.write( 0 );        //LSB
  Wire.endTransmission();
  delay(10);

  //Propagation velocities menu ------------------------------------------------------------
  while (digitalRead(button_OK)) ;
  ypos = 10;
  tft.setCursor(xpos, ypos);
  tft.setTextSize(1);
  tft.println("Select propagation velocity:");
  ypos += 13;//tft.fontHeight(GFXFF);
  //tft.setTextSize(1);
  for (i = 0; i < (sizeof(propagation_media) / sizeof(propagation_media[0])); i++)
  {
    tft.setCursor(xpos, ypos);
    tft.println(propagation_media[i]);
    ypos += 13;//tft.fontHeight(GFXFF);
  }
  i = 0;
  ypos = 20 + i * 11;//tft.fontHeight(GFXFF);
  tft.drawRect(0, ypos, 320, 12, ST77XX_RED); //Draw cursor
  cont = false;
  do {
    while (digitalRead(button_up) && digitalRead(button_down) && digitalRead(button_OK)) {
      ; // wait
    }
    delay(10);
    if (digitalRead(button_down))
    {
      if (i < (sizeof(propagation_media) / sizeof(propagation_media[0]) - 1)) {
        i++;
      }
      else {
        i = 0;
      }
      tft.drawRect(0, ypos, 320, 12, ST77XX_BLACK);
      ypos = 20 + i * 13;//tft.fontHeight(GFXFF);
      tft.drawRect(0, ypos, 320, 12, ST77XX_RED);
      delay(500);
    }
    else if (digitalRead(button_up))
    {
      if (i > 0) {
        i--;
      }
      else {
        i = (sizeof(propagation_media) / sizeof(propagation_media[0]) - 1);
      }
      tft.drawRect(0, ypos, 320, 12, ST77XX_BLACK);
      ypos = 20 + i * 13;//tft.fontHeight(GFXFF);
      tft.drawRect(0, ypos, 320, 12, ST77XX_RED);
      delay(500);
    }
    else if (digitalRead(button_OK))
    {
      c = velocities[i];
      cont = true;
    }
  } while (!cont);
  resolution = c / (2.0 * bandwidth);                  //depth resolution [m]
  min_depth = -1.0 * float(cable_offset) * resolution; //offset cables + antennas

  

  //Depth menu--------------------------------------------------------------
  i = 0;
  word max_depth_index = sizeof(depths) / sizeof(depths[0]) - 1;

  tft.fillScreen(ST77XX_BLACK);
  while (digitalRead(button_OK)) ;
  ypos = 10;
  tft.setCursor(xpos, ypos);
  tft.setTextSize(1);
  tft.println("Select maximum displayed depth:");
  ypos += 13;
  for (i = 0; i <= max_depth_index; i++)
  {
    double temp_depth = depths[i] * resolution + min_depth;
    double no_of_decimal_places = temp_depth - floor(temp_depth);
    String temp_str;
    if (no_of_decimal_places < (temp_depth / 10)) temp_str = String(temp_depth, 0) + " m";
    else temp_str = String(temp_depth, 1) + " m";
    char text[6];
    temp_str.toCharArray(text, 6);
    tft.setCursor(xpos, ypos);
    tft.setTextSize(GFXFF);
    tft.println(text);
    ypos += 13;//tft.fontHeight(GFXFF);
  }
  i = 0;
  ypos = 20 + i * 11;//tft.fontHeight(GFXFF);
  tft.drawRect(0, ypos, 320, 12, ST77XX_RED); //Draw cursor
  cont = false;
  delay(1000);
  do {
    while (digitalRead(button_up) && digitalRead(button_down) && digitalRead(button_OK)) {
      ; // wait
    }
    delay(10);
    if (digitalRead(button_down))
    {
      if (i < max_depth_index) {
        i++;
      }
      else {
        i = 0;
      }
      tft.drawRect(0, ypos, 320, 12, ST77XX_BLACK);
      ypos = 20 + i * 13;//tft.fontHeight(GFXFF);
      tft.drawRect(0, ypos, 320, 12, ST77XX_RED);
      delay(500);
    }
    else if (digitalRead(button_up))
    {
      if (i > 0) {
        i--;
      }
      else {
        i = max_depth_index;
      }
      tft.drawRect(0, ypos, 320, 12, ST77XX_BLACK);
      ypos = 20 + i * 13;//tft.fontHeight(GFXFF);
      tft.drawRect(0, ypos, 320, 12, ST77XX_RED);
      delay(500);
    }
    else if (digitalRead(button_OK))
    {
      no_res_cells_vert = depths[i];
      max_depth = no_res_cells_vert * resolution + min_depth;
      double depth_step_temp = (max_depth - min_depth) / 4;
      i = 0;
      while ((abs(depth_step_temp - depth_steps[i]) > abs(depth_step_temp - depth_steps[i + 1])) && ((i + 1) < (sizeof(depth_steps) / sizeof(depth_steps[0]) - 1)))
      {
        i++;
      }
      if (abs(depth_step_temp - depth_steps[i]) > abs(depth_step_temp - depth_steps[i + 1])) i++;
      depth_step = depth_steps[i];
      no_depth_steps_decimal_places = depth_steps_decimal_places[i];
      cont = true;
    }
  } while (!cont);

  //Distance menu--------------------------------------------------------------
  tft.fillScreen(ST77XX_BLACK);
  while (digitalRead(button_OK)) ;
  ypos = 10;
  //tft.setFont(FSB12);
  tft.setCursor(xpos, ypos);
  tft.setTextSize(1);
  tft.println("Select horizontal distance on screen:");
  ypos += 13;//tft.fontHeight(GFXFF)
  for (i = 0; i < (sizeof(distances) / sizeof(distances[0])); i++) {
    String temp_str = String(distances[i]) + " m";
    char text[5];
    temp_str.toCharArray(text, 5);
    tft.setCursor(xpos, ypos);
    tft.setTextSize(GFXFF);
    tft.println(text );
    ypos += 13;//tft.fontHeight(GFXFF)
  }
  i = 0;
  ypos = 20 + i * 13;//tft.fontHeight(GFXFF)
  tft.drawRect(0, ypos, 320, 12, ST77XX_RED); //Draw cursor
  cont = false;
  delay(1000);
  do {
    while (digitalRead(button_up) && digitalRead(button_down) && digitalRead(button_OK)) {
      ; // wait
    }
    delay(10);
    if (digitalRead(button_down))
    {
      if (i < (sizeof(distances) / sizeof(distances[0]) - 1)) {
        i++;
      }
      else {
        i = 0;
      }
      tft.drawRect(0, ypos, 320, 12, BLACK);
      ypos = 20 + i * 13; // * tft.fontHeight(GFXFF);
      tft.drawRect(0, ypos, 320, 12, RED);
      delay(500);
    }
    else if (digitalRead(button_up))
    {
      if (i > 0) {
        i--;
      }
      else {
        i = (sizeof(distances) / sizeof(distances[0]) - 1);
      }
      tft.drawRect(0, ypos, 320, 12, BLACK);
      ypos = 20 + i * 13;
      tft.drawRect(0, ypos, 320, 12, RED);
      delay(500);
    }
    else if (digitalRead(button_OK))
    {
      max_dist = distances[i];
      dist_step = max_dist / 6;
      cont = true;
    }
  } while (!cont);

  //graph parameters
  res_cell_height = graph_height / no_res_cells_vert;  	  			      //[pixels]
  no_res_cells_horiz = max_dist / GPR_horizontal_step;
  res_cell_width = graph_width / no_res_cells_horiz;                   //[pixels]
  // Draw grid
  tft.fillScreen(BLACK);
  Graph(tft, orig_x, orig_y, graph_width, graph_height, 0, max_dist, dist_step, min_depth, max_depth, depth_step, "GPR", "Distance [m]", "Depth [m]");
  disp_card_status();
  check_displ_batlevel();
  disp_GPS_status();
  //attachInterrupt(digitalPinToInterrupt(HallSensor), wheelStep, FALLING);
}

//------------------------------------------------------------------------
//12388.8

void loop() {
  //GPS reading
  if(firstRun == false){
    for (int i = 0; i < NUMPIXELS; i++) { // Para cada pixel...
     pixels.setPixelColor(i, pixels.Color(0, 255, 0));
    }
    pixels.show();
    if (card){
      trackLogFile = SD.open("trk-log.txt", FILE_READ);
    //Verify file existance, if missing, create it and write speed index
      if (trackLogFile ) {
        Serial.println("trk-log opened");
        String cadena = "";
        Serial.print("last track index:");
        while(trackLogFile.available()){
          char car= trackLogFile.read(); 
          cadena += car;
        }
        logIndex = cadena.toInt();
        Serial.println(logIndex);
        trackLogFile.close(); 
        char buffLog[4];
        sprintf(buffLog, "%03d", logIndex);
        logName = "trk-"+String(buffLog)+".gpr";
        //Serial.print("new log file name:");
        //Serial.println(logName);
      }
      SD.remove("trk-log.txt");
      trackLogFile = SD.open("trk-log.txt", FILE_WRITE);
      if (trackLogFile ) {
        trackLogFile.println(logIndex+1);
        trackLogFile.close();  
      }
      
    }
    if (card){
      
      //Verify file existance, if missing, create it and write speed index
      if (!SD.exists(logName)) {
  
        file = SD.open(logName, FILE_WRITE);
        Serial.println("#"+logName+" created");
        file.write(highByte(i));
        file.write(lowByte(i));
        Serial.println("First data written");
        file.close();
      }
    }
    firstRun = true; 
  }  
  
  while (Serial3.available()) {
    int c = Serial3.read();
    if (gps.encode(c)) {

      gps.f_get_position(&latitude, &longitude);
      GPSfixed = true;
    }
  }


  // Wait for step wheel signal
  while (digitalRead(HallSensor) == 0)
  {
    check_displ_batlevel();
    disp_GPS_status();
  }
  while (digitalRead(HallSensor) == 1)
  {
    check_displ_batlevel();
    disp_GPS_status();
  }
  Serial.print("@Lat/Lon:");
  Serial.print(latitude,6);
  Serial.print(",");
  Serial.println(longitude,6);

  Serial.println("&capture-still");

  // If screen is full, delete and start again
  if (((scan_index % no_res_cells_horiz) == 0) && (scan_index != 0))
  {
    word nr_ecran = scan_index / no_res_cells_horiz;
    tft.fillScreen(BLACK);
    Graph(tft, orig_x, orig_y, graph_width, graph_height, nr_ecran * max_dist, (nr_ecran + 1) * max_dist, dist_step, min_depth, max_depth, depth_step, "GPR", "Distance [m]", "Depth [m]");
    disp_card_status();
    check_displ_batlevel();
    disp_GPS_status();
  }
  // Generate modulation signal and sample radar echo
  for (i = 0; i <= 4080; i = i + 16) // DAC goes from 0 - 4.98V
  {
    // Write to DAC
    Wire.beginTransmission(MCP4725_Address);
    Wire.write( highByte(i) );        //MSB
    Wire.write( lowByte(i) );         //LSB
    Wire.endTransmission();
    // Read from ADC
    samples[i >> 4] = analogRead(pin_ADC); // >>4 means /16
  }
  //Bring DAC to 0
  Wire.beginTransmission(MCP4725_Address);
  Wire.write( 0 );        //MSB
  Wire.write( 0 );        //LSB
  Wire.endTransmission();

  //Store on SD Card
  if (card)
  {
    file = SD.open(logName, FILE_WRITE);
    byte bufferLat[4];
    byte bufferLon[4];
    memcpy(bufferLat, &latitude, 4);
    memcpy(bufferLat, &longitude, 4);
    char identifier = '$';
    char separator = '&';
    file.write(identifier); 
    file.write(bufferLat, 4);
    file.write(bufferLon, 4);
    file.write(separator);
    for (i = 0; i < no_of_samples; i++)
    {
      file.write(highByte(samples[i]));
      file.write(lowByte(samples[i]));
      //file.flush();
      // Serial.println(highByte(samples[i]));
      //Serial.println(lowByte(samples[i]));


    }
    file.close();
  }

  // Prepare data for FFT
  for (i = 0; i < no_of_samples; i++)
  {
    //real[i] = (double)(samples[i]) - (double)(correction[i]); // Load samples and correct antenna coupling
    real[i] = (double)(samples[i]) - 512.0; // Load samples and eliminate d.c.
    imag[i] = 0.0;                          // Delete imaginary part
  }
  // Compute FFT
  //FFT.Windowing(real, no_of_samples, FFT_WIN_TYP_HAMMING, FFT_FORWARD); //Windowing
  FFT.Compute(real, imag, no_of_samples, FFT_FORWARD); //FFT
  FFT.ComplexToMagnitude(real, imag, no_of_samples);   //Compute FFT and store it in real
  //Draw one column
  for (i = 0; i < no_res_cells_vert; i++)
  {
    depth_corrected_amplitude = real[i] * exp(i * 0.025);   // depth correction
    scaled_amplitude = (word)(depth_corrected_amplitude * 255.0 / max_amplitude);
    if (scaled_amplitude > 255) scaled_amplitude = 255;
    color = (((scaled_amplitude & 0b11111000) << 8) + ((scaled_amplitude & 0b11111100) << 3) + (scaled_amplitude >> 3));
    tft.fillRect(orig_x + 1 + scan_index % no_res_cells_horiz * res_cell_width, orig_y + 1 - graph_height + i * res_cell_height, res_cell_width, res_cell_height, color);
  }
  scan_index++;
}



// Grid drawing routine
//
// list of arguments
// &tft display object
// gx = graph origin x (bottom left)
// gy = graph origin y (bottom left)
// w = graph width (width)
// h = graph height (height)
// xlo = lower limit axis x
// xhi = upper limit x axis
// xinc = increase (step) x axis
// ylo = lower limit y axis
// yhi = upper limit y axis
// yinc = increase (step) y axis
// title = graph title
// xlabel = x-axis label
// ylabel = y axis label

void Graph(Adafruit_ST7789 & tft, double gx, double gy, double w, double h, double xlo, double xhi, double xinc, double ylo, double yhi, double yinc, char *title, char *xlabel, char *ylabel)
{
  double ydiv, xdiv;
  double i;
  double temp;
  int rot, newrot;

  unsigned int gcolor = ST77XX_BLUE; //grid color
  unsigned int acolor = ST77XX_RED;    //axis color
  unsigned int tcolor = ST77XX_WHITE;  //text color
  unsigned int bcolor = ST77XX_BLACK;  //background color


  // draw x axis
  tft.drawLine(gx, gy - h, gx + w, gy - h, acolor);  //red axis
  tft.setTextColor(acolor, bcolor);
  tft.setCursor((int)(gx + w - 60) , (int)(gy - h) - 12);
  tft.setTextSize(1);
  tft.println(xlabel);
  // draw origin
  tft.setTextColor(tcolor, bcolor);
  tft.setCursor(gx - 35, gy - h);
  tft.setTextSize(1);
  tft.println(ylo);

  for (i = 0; i <= yhi; i += yinc) {
    temp =  gy - h + (i - ylo) * h / (yhi - ylo);
    tft.drawLine(gx, temp, gx + w, temp, gcolor);
    // draw y axis labels
    tft.setTextColor(tcolor, bcolor);
    tft.setCursor(gx - 25, temp);
    tft.setTextSize(1);
    tft.println(i);
  }
  tft.drawLine(gx, gy, gx + w, gy, gcolor);//graph bottom line

  // draw y axis
  for (i = xlo; i <= xhi; i += xinc)
  {
    temp =  (i - xlo) * w / (xhi - xlo) + gx;
    if (i == xlo) 									//red axis
    {
      tft.drawLine(temp, gy, temp, gy - h, acolor);
      tft.setTextColor(acolor, bcolor);
      tft.setCursor((int)temp, (int)(gy - h - 12));
      tft.setTextSize(1);
      tft.println(ylabel);
    }
    else
    {
      tft.drawLine(temp, gy, temp, gy - h + 1, gcolor);
    }
    //draw x axis labels
    tft.setTextColor(tcolor, bcolor);
    tft.setCursor(temp, gy + 7);
    tft.setTextSize(1);
    tft.println((int)i);
  }

  //draw graph label
  tft.setTextColor(tcolor, bcolor);
  tft.setCursor((int)(gx + w / 2) , (int)(gy - h - 12));
  tft.setTextSize(1);
  tft.println(title);
}

void disp_GPS_status() {

  if (GPSfixed) {
    tft.setTextColor(ST77XX_GREEN);
    tft.setCursor(100, 10);
    tft.setTextSize(1);
    tft.print("GPS FIXED");
  }
  else
  {
    tft.setTextColor(ST77XX_RED);
    tft.setCursor(100, 10);
    tft.setTextSize(1);
    tft.println("NO GPS");
  }

}
void disp_card_status()
{
  if (card)
  {
    tft.setTextColor(ST77XX_GREEN);
    tft.setCursor(10, 10);
    tft.setTextSize(1);
    tft.print("CARD OK");
  }
  else
  {
    tft.setTextColor(ST77XX_RED);
    tft.setCursor(10, 10);
    tft.setTextSize(1);
    tft.println("NO CARD");
  }
}

void check_displ_batlevel(void)
{
  //Measure and display battery state
  word val = analogRead(pin_BAT);
  if (val <= battery_level[0]) // low
  {
    tft.drawRect(290, 5, 20, 12, ST77XX_RED);//-140
    tft.drawRect(431 - 140, 6, 18, 10, ST77XX_RED);
    tft.fillRect(450 - 140, 7, 3, 8, ST77XX_RED);
    tft.fillRect(432 - 140, 7, 16, 8, ST77XX_BLACK);
  }
  else if (val <= battery_level[1]) // med1
  {
    tft.drawRect(430 - 140, 5, 20, 12, ST77XX_WHITE);
    tft.fillRect(450 - 140, 7, 3, 8, ST77XX_WHITE);
    tft.fillRect(432 - 140, 7, 5, 8, ST77XX_WHITE);
    tft.fillRect(437 - 140, 7, 11, 8, ST77XX_BLACK);
  }
  else if (val <= battery_level[2]) // med2
  {
    tft.drawRect(430 - 140, 5, 20, 12, ST77XX_WHITE);
    tft.fillRect(450 - 140, 7, 3, 8, ST77XX_WHITE);
    tft.fillRect(432 - 140, 7, 11, 8, ST77XX_WHITE);
    tft.fillRect(443 - 140, 7, 5, 8, ST77XX_BLACK);
  }
  else // max
  {
    tft.fillRect(430 - 140, 5, 20, 12, ST77XX_WHITE);
    tft.fillRect(450 - 140, 7, 3, 8, ST77XX_WHITE);
  }
}
